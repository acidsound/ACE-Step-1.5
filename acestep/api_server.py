        ...
        for item in os.listdir(self.lora_root):
            ...
            if os.path.isdir(path):
                ...
                if os.path.exists(config_path):
                    loras.append(
                        {"id": item, "path": path, "created_at": os.path.getctime(path)}
                    )
...
def create_app() -> FastAPI:
        ...
        async def _run_one_job(job_id: str, req: GenerateMusicRequest) -> None:
            ...
            if req.lora_id:
                ...
                if lora_path:
                    ...
                    print(
                        f"[API Server] Job {job_id}: LoRA {req.lora_id} not found, ignoring"
                    )
            elif h.lora_loaded:
            ...
            def _blocking_generate() -> Dict[str, Any]:
                ...
                # Track seeds used for each audio
                actual_seed_list = []
                ...
                for run_idx in range(sequential_runs):
                    ...
                    p_start = run_idx / sequential_runs
                    ...
                    # Collect seeds from each run
                    for audio in result.audios:
                        if audio.get("params", {}).get("seed") is not None:
                            actual_seed_list.append(str(audio["params"]["seed"]))
                ...
                if lora_loaded_for_this_job:
                    print(
                        f"[API Server] Unloading LoRA {req.lora_id} for job {job_id}"
                    )
                    h.unload_lora()
                ...
                seed_value = ",".join(actual_seed_list) if actual_seed_list else ""
            ...
            if rec:
                ...
                if rec.result and rec.status == "succeeded":
                    ...
                    if (
                        ...
                        original_prompt = metas_raw.get(
                            "prompt", metas_raw.get("caption", "")
                        )
                        original_lyrics = metas_raw.get("lyrics", "")
    ...
    async def transcribe_prompt_endpoint(
        request: Request, authorization: Optional[str] = Header(None)
    ):
    ...
    async def understand_music_endpoint(
        ...
        try:
            ...
            result = understand_music(llm_handler=llm, audio_codes=audio_codes)
    ...
    async def upload_lora_endpoint(
        ...
        try:
            ...
            return _wrap_response(
                {"lora_id": lora_id, "status": "registered", "path": actual_path}
            )
        finally:
